...
if (!empty($self->_width))
  echo <<<EOF
#mainBody { width:$self->_width;};
EOF;

if (!$self->_uppergoform)
  echo <<<EOF
...
<?php endif; /* topbanner */?>
<?php if ($self->_toptitle) echo $title;?></div>
<?php if ($self->_topbanner): ?>
 </div>
</div>
<?php endif; /* topbanner */?>
...
#toggle-handle {
	...
	height: 15px;
	float: left;
	background: url(<?php echo $self->themeurl?>/imgs/toggle.png) no-repeat;
}
...
function switch_menu()
{
	...
	var img = "url('<?php echo $self->themeurl?>/imgs/bg_sidemenu.png')";
...
if ($options['id']=='Anonymous')
  ...
  $pg = $options['id'];
echo macro_calendar($self,"'$pg',blog,noweek,archive,center",$pg);
echo '</div>';
endif;

if (!empty($DBInfo->use_scrap)) {
  echo "<div class='scrap'>";
  echo macro_Scrap($self);
  echo "</div>";
}
if ($self->_submenu==1) :
  echo '<div id="subMain">';
...
if ($self->_randomquote==1) :
echo '<div class="randomQuote">';
echo macro_RandomQuote($self);
echo '</div>';
endif;
echo '<div class="randomPage">';
echo macro_RandomPage($self,"4,simple");
echo '</div>';
if (!empty($DBInfo->use_tagging)) {
  echo "<div>";
  echo macro_Keywords($self,"all,tour,limit=15");
  echo "</div>";
}
?>
</div>
...
<div id='mycontent'>
<?php if (empty($self->_toptitle)) echo '<div class="wikiTitle" id="wikiTitle">'.$title.'</div>';?>
<?php echo $subindex;?>
